...
Report-then-apply: present candidates, let the user pick, then grill the chosen one with inline
doc side-effects. Uses the shared vocabulary `${CLAUDE_PLUGIN_ROOT}/shared/deep-modules.md`
(module, interface, depth, seam, deletion test); fuller architecture layer over it — substitution
bans + adapter/leverage/locality — in [LANGUAGE.md](references/LANGUAGE.md). Use these terms
exactly — don't drift to "component/service/boundary".
...
1. **Explore.** Read the domain glossary + stances in the area first (don't re-litigate them;
   multi-package fan-out: `${CLAUDE_PLUGIN_ROOT}/shared/context-doc.md`). Use the `investigator`
   agent (or `Explore`) to walk the codebase noting **friction**: concept
...
2. **Present candidates as a self-contained HTML report** in the OS temp dir (never the repo); full
   scaffold (CDN deps, badges, before/after, Top-recommendation): [HTML-REPORT.md](references/HTML-REPORT.md).
